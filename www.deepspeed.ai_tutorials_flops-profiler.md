...
URL Source: https://www.deepspeed.ai/tutorials/flops-profiler/

Published Time: 2025-07-23T16:32:25-07:00

Markdown Content:
In this tutorial, we introduce the DeepSpeed Flops Profiler and provide examples of its usage.

*   [Overview](https://www.deepspeed.ai/tutorials/flops-profiler/#overview)
*   [Flops Measurement](https://www.deepspeed.ai/tutorials/flops-profiler/#flops-measurement)
*   [Multi-GPU, Multi-node, Data Parallelism, and Model Parallelism](https://www.deepspeed.ai/tutorials/flops-profiler/#multi-gpu-multi-node-data-parallelism-and-model-parallelism)
*   [Usage](https://www.deepspeed.ai/tutorials/flops-profiler/#usage)
...
For models running on multi-GPU or multi-node, only change of the model parallelism (e.g., `--model-parallel-size` in [Megatron-LM](https://github.com/NVIDIA/Megatron-LM)) affects the number of flops and parameters profiled, i.e., `model_parallel_size * flops = total_flops` and `model_parallel_size * parameters = total_parameters`. The data parallel size or world size (related to the number of GPUs or nodes) does not affect the per GPU profile.

Usage
-----

The DeepSpeed Flops Profiler can be used with the DeepSpeed runtime or as a standalone package. When using DeepSpeed for model training, the profiler can be configured in the deepspeed [configuration file](https://www.deepspeed.ai/docs/config-json/#flops-profiler) without user code changes. To use the flops profiler outside the DeepSpeed runtime, install DeepSpeed and import the`flops_profiler`package to use the APIs directly. Examples of each usage are given below.

*   [Usage With the DeepSpeed Runtime](https://www.deepspeed.ai/tutorials/flops-profiler/#usage-with-the-deepspeed-runtime)
    *   [Example: Megatron-LM](https://www.deepspeed.ai/tutorials/flops-profiler/#example-megatron-lm)

*   [Usage Outside the DeepSpeed Runtime](https://www.deepspeed.ai/tutorials/flops-profiler/#usage-outside-the-deepspeed-runtime)
    *   [In Model Inference](https://www.deepspeed.ai/tutorials/flops-profiler/#in-model-inference)
        *   [Example: AlexNet](https://www.deepspeed.ai/tutorials/flops-profiler/#example-alexnet)
        *   [Example: Bert](https://www.deepspeed.ai/tutorials/flops-profiler/#example-bert)

    *   [In Model Training Workflow](https://www.deepspeed.ai/tutorials/flops-profiler/#in-model-training-workflow)
        *   [Example Training Workflow](https://www.deepspeed.ai/tutorials/flops-profiler/#example-training-workflow)

### Usage With the DeepSpeed Runtime

When using DeepSpeed for model training, the profiler can be configured in the deepspeed [configuration file](https://www.deepspeed.ai/docs/config-json/#flops-profiler). No explicit API calls are needed to use the profiler. The profiler can be enabled by adding the following field to deepspeed’s configuration json file. Refer to [flops profiler](https://www.deepspeed.ai/docs/config-json/#flops-profiler) for details.
...
The profiler can be used as a standalone package outside of the DeepSpeed runtime. One can simply install DeepSpeed and import the `flops_profiler` package to use the APIs directly. Refer to [installation of DeepSpeed](https://www.deepspeed.ai/getting-started/#installation) for installing DeepSpeed.
...
To profile model forward in a training workflow, use the `FlopsProfiler`class. The `FlopsProfiler`class provides the following methods:
